...
BANNER = '''
  Usage: jdustc [options] path/to/script.jade -- [args]
'''
...
SWITCHES = [
  ...
  ['-j', '--join [FILE]',     'concatenate the source Jade before compiling']
...
exports.run = ->
  ...
  return usage()                         if opts.interactive
...
compilePath = (source, topLevel, base) ->
  fs.stat source, (err, stats) ->
    ...
    if err?.code is 'ENOENT'
      if topLevel and source[-7..] isnt '.jade'
        source = sources[sources.indexOf(source)] = "#{source}.jade"
    ...
    else if topLevel or path.extname(source) is '.jade'
...
compileScript = (file, input, base) ->
  ...
  try
    ...
    if      o.tokens      then printTokens CoffeeScript.tokens t.input
      ...
      fn = jade.compile(t.input, {filename:"."})
      input = fn({filename:"."})
      t.output = dust.compile(input, t.file.split("/")[-1..][0].split(".")[0])#CoffeeScript.compile t.input, t.options
